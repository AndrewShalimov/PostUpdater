...
function findFileAtCatalog(fileName) {
    var index = -1;
    if(!catalogBase || catalogBase.length == 0) {
        logger.error('Catalog is empty! Exiting.');
        process.exit();
    }
    for (var i = 0; i < catalogBase.length; i++) {
        if (catalogBase[i] != undefined) {
...
function wordPressUpdater(fileKeyForSearch, newEmbedLink, error) {
    ...
    var percentIndex = fileKeyForSearch.indexOf("%");
    if (percentIndex > -1) {
        fileKeyForSearch = fileKeyForSearch.slice(fileKeyForSearch.lastIndexOf('%') + 2, fileKeyForSearch.length);
    }
    wp.posts()
        .param('context', 'edit')
        ...
        .then(function (posts) {
            ...
            if (posts.length > 1) {
                var infoLine = "Search key '" + fileKeyForSearch + "' was found with more then 1 post. Update it manually.";
                logger.info(infoLine);
                failedFileNames.push(infoLine);
                restoreAbuseFile();
                return;

...
function getMessage(uid, callback) {
    mailClient.listMessagesByUID(uid, uid, callback);
}
...
function wordPressUpdater(fileKeyForSearch, newEmbedLink) {
    wp.posts()
        .param('context', 'edit')
        ...
        .then(function (posts) {
            if (posts.length == 0) {
                logger.info("Post with key '" + fileKeyForSearch + "' was NOT found in WordPress.");
                readyToNextFile = true;
            } else {
                updatePost(posts[0], newEmbedLink);
            }
        },
        function (err) {
            logger.error(err);
...
function updatePost(post, newEmbedLink) {
    ...
    wp.posts().id(postId).update({
        ...
        function (response) {
            var infoLine = 'Post ' + postId + ' updated successfully.';
